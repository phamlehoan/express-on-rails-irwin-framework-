  ...
  <div class="container-fluid">
    ...
    <div class="card mb-4 content-card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h6 class="mb-0 text-uppercase text-primary fw-bold">{{ t('admin.feature_information') }}</h6>
        <div class="d-flex gap-2" v-if="canEdit">
          ...
          <button v-if="isEdit" class="btn btn-sm btn-primary" @click="saveFeature">
            <i class="fa fa-save me-1" aria-hidden="true"></i>
            {{ t('save') }}
          </button>
          <form v-if="!isEdit" class="d-inline" method="post" :action="`/admin/features/${feature.id}?_method=DELETE`" @submit.prevent="onDeleteSubmit">
        ...
        <template v-if="!isEdit">
          <div class="row">
            ...
            <div class="col-md-3 mb-3">
              ...
              <p class="mb-0">
                <span class="badge" :class="feature.type === 'MENU_GROUP' ? 'bg-secondary' : 'bg-info'">{{ feature.type }}</span>
              </p>
            </div>
            <div class="col-md-3 mb-3">
              <label class="text-muted small">{{ t('admin.sort_order') }}</label>
            ...
            <div class="col-12 mb-3" v-if="feature.description">
              <label class="text-muted small">{{ t('admin.description') }}</label>
        ...
        <form v-else @submit.prevent="saveFeature">
          <div class="row">
            ...
            <div class="col-md-4 mb-3">
              ...
              <input class="form-control" type="number" v-model.number="form.sortOrder" min="0" />
            </div>
    ...
    <div class="card mb-4 content-card">
      ...
      <div class="card-body">
        <ul class="list-unstyled mb-0" v-if="feature.permissions && feature.permissions.length">
...
export default defineComponent({
  ...
  computed: {
    ...
    parentName() {
      ...
      const p = this.features.find((f: { id: string }) => f.id === this.feature.parentId);
      return p ? p.name : '-';
    },
    parentOptions() {
      return this.features.filter((f: { id: string }) => f.id !== this.feature.id);
    },
  },
  methods: {
    toggleEdit() {
    ...
    saveFeature() {
      ...
      ['code', 'name', 'description', 'parentId', 'sortOrder'].forEach((k) => {
        ...
        i.name = k;
        i.value = k === 'sortOrder' ? String(this.form.sortOrder) : (this.form[k as keyof typeof this.form] || '');
