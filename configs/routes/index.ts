...
export class Route {
  ...
  public static resource(
    ...
    if (
      this.isAllowAccess(options?.only, options?.except, RestActions.Create)
    ) {
      if (options?.setPermissionFor) {
        const permissionMiddleware = new ValidateUserPermissionMiddleware(
          `${options?.setPermissionFor}::${Permission.Create}`
    ...
    if (this.isAllowAccess(options?.only, options?.except, RestActions.Edit)) {
      if (options?.setPermissionFor) {
        ...
        path
          .route("/:id/edit")
          .get(customController.edit.bind(customController));
      }
    }

    if (
      this.isAllowAccess(options?.only, options?.except, RestActions.Update)
    ) {
      if (options?.setPermissionFor) {
        const permissionMiddleware = new ValidateUserPermissionMiddleware(
          `${options?.setPermissionFor}::${Permission.Update}`
    ...
    if (
      this.isAllowAccess(options?.only, options?.except, RestActions.Destroy)
    ) {
      if (options?.setPermissionFor) {
        const permissionMiddleware = new ValidateUserPermissionMiddleware(
        ...
        path
          .route("/:id")
          .delete(customController.destroy.bind(customController));
      }
    }
  }
